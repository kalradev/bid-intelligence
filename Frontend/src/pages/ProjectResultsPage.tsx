...
export default function ProjectResultsPage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(160deg, rgba(232,248,245,0.6) 0%, rgba(234,239,239,0.5) 50%, rgba(165,233,221,0.2) 100%)" }}>
      <DashboardNavbar />

      <div style={{ maxWidth: 800, margin: "0 auto", padding: `${NAVBAR_HEIGHT + 24}px 24px 32px` }}>
        {loading && <div style={{ textAlign: "center", padding: 48, color: "#34908B" }}>Loading project result…</div>}
        {error && !loading && (
          <div style={{ padding: 24, background: "rgba(254,226,226,0.8)", color: "#dc2626", borderRadius: 12, marginBottom: 24, border: "1px solid rgba(111,190,178,0.3)" }}>
            {error}
          </div>
        )}
        {analysis && !loading && (
          <div style={{ background: "#fff", borderRadius: 16, padding: 28, boxShadow: "0 4px 16px rgba(0,0,0,0.06)", border: "1px solid rgba(111,190,178,0.2)" }}>
            <h1 style={{ margin: "0 0 8px", fontSize: 22, color: "#1e293b", display: "flex", alignItems: "center", gap: 8 }}>
              <FileText size={24} style={{ color: "#6FBEB2" }} /> {analysis.projectName} – Result (read-only)
            </h1>
            ...
            <div style={{ marginTop: 24 }}>
              <h2 style={{ margin: "0 0 12px", fontSize: 16, fontWeight: 700, color: "#34908B" }}>Project overview</h2>
              <div style={{ display: "grid", gap: 12 }}>
                {details.map(({ label, value }) => (
                  <div key={label} style={{ display: "flex", justifyContent: "space-between", padding: "10px 14px", background: "rgba(165,233,221,0.08)", borderRadius: 10, border: "1px solid rgba(111,190,178,0.15)" }}>
            ...
            <div style={{ marginTop: 28 }}>
              <button
                ...
                style={{
                  ...
                  cursor: "pointer",
                  fontSize: 15,
                  boxShadow: "0 2px 8px rgba(111,190,178,0.3)",
                  transition: "all 0.2s ease",
                }}
                onMouseEnter={(e) => {
                  e.currentTarget.style.background = "#34908B";
                  e.currentTarget.style.boxShadow = "0 4px 12px rgba(111,190,178,0.4)";
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.background = "#6FBEB2";
                  e.currentTarget.style.boxShadow = "0 2px 8px rgba(111,190,178,0.3)";
                }}
              >
        ...
        <div style={{ marginTop: 28, textAlign: "center" }}>
          <button
            ...
            onMouseEnter={(e) => {
              ...
              e.currentTarget.style.boxShadow = "0 4px 12px rgba(111,190,178,0.25)";
            }}
            onMouseLeave={(e) => {
