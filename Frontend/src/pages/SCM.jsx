...
const SCM = () => {
  ...
  useEffect(() => {
    ...
    if (storedData) {
      ...
      if (scmData) {
        setData(processDepartmentData(scmData));
      } else {
        setData(null);
      }
    }
  ...
  useEffect(() => {
    ...
    const key = `riskAreas_user_scm_${projectName}_${documentId ?? ""}`;
    try {
  ...
  useEffect(() => {
    ...
    localStorage.setItem(`riskAreas_user_scm_${projectName}_${documentId ?? ""}`, JSON.stringify(userRiskAreas));
